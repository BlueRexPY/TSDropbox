...
const Path = (SerchPath: CurrentPath) => {
  ...
  useEffect(() => {
    if(!AuthStore.AuthSettings.auth){
      router.push("/")
    }else{FolderService.fetchData(SerchPath.path.slice(1,SerchPath.path.length).replace(/_/g, "/")).then(res=>setitemList(res))}
  ...
  return (
    <Layout title='Files'>
      ...
      <ListGroup>
        {itemList.map((e)=>{return <FileItem key={e.id} item={e}/>})}
      </ListGroup>
    </Layout>
  )
}
